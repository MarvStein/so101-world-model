...
def read_and_process_video(
    ...
    if resize:
        C, H, W = (
            extracted_frames.shape[1],
            extracted_frames.shape[2],
            extracted_frames.shape[3],
        )
        target_h, target_w = resolution
...
class Video2WorldPipeline(BasePipeline):
    ...
    def from_config(
        ...
        pipe._cached_zero_encoding = None

        # 4. Load text encoder
        if use_text_encoder:
            pipe.text_encoder = CosmosT5TextEncoder(
                config=config.text_encoder.t5,
    ...
    def denoise(
        ...
        """
        if xt_B_C_T_H_W.shape[2] != self.config.state_t:
            msg = f"Got incorrect num frames {xt_B_C_T_H_W.shape=}."
            raise ValueError(msg)
        ...
        if condition.is_video:
            ...
            if self.config.conditioning_strategy == str(ConditioningStrategy.CHANNEL_CONCAT):
                net_state_in_B_C_T_H_W = torch.cat(
                    [net_state_in_B_C_T_H_W, torch.zeros_like(net_state_in_B_C_T_H_W)],
                    dim=1,
                )
        ...
        if return_only_hidden_states_up_to is not None:
            net_output_B_C_T_H_W, hidden_states_B_T_H_W_D = net_output_B_C_T_H_W
        else:
            hidden_states_B_T_H_W_D = None

        if net_output_B_C_T_H_W is not None:
            ...
            x0_pred_B_C_T_H_W, eps_pred_B_C_T_H_W = None, None

        if return_decoded_video:
            decoded_video = self.decode(x0_pred_B_C_T_H_W[:1])[0].clamp(-1.0, 1.0)
        else:
            decoded_video = None
        ...
        return DenoisePrediction(
            ...
            logvar=None,
            hidden_states=hidden_states_B_T_H_W_D,
            sample_decoded_video=decoded_video,
        )
    ...
    def get_x0_fn_from_batch(
        ...
        def x0_fn(
            ...
            if return_only_hidden_states_up_to is not None:
                return raw_x0, cond.hidden_states
    ...
    def generate_video(
        ...
        *,
        ...
        for i, _ in enumerate(tqdm(scheduler.timesteps, desc="Generating world", leave=False)):
            ...
            x0_pred = x0_fn(sample, sigma_in, return_only_hidden_states_up_to=return_only_hidden_states_up_to)
            if return_only_hidden_states_up_to is not None:
        ...
        return_hidden_states = return_all_context or i + 1 == return_context_at_step
        samples = x0_fn(
            sample, sigma_in, return_only_hidden_states_up_to=float("inf") if return_hidden_states else None
        )
        if return_hidden_states:
            samples, hidden_states = samples
    ...
    def __call__(
        ...
        assert self.config.resolution == "480"
